...
public class Knightdragon
{
	...
	private static void print(int rounds, String choise, int dragonHP)
	{
		...
		System.out.println("    " + choise + "\t\t" + dragonHP + "\t\t         " + dragonA 
				+ "\t\t\t" + knightHP + "\t\t\t" + knightA);
		System.out.println();
	}
	...
	private static int dragon(int dragonHP)
	{
		...
		else if ((dragonHP - knightA <= 0) && !(knightHP <= dragonA + buff))
		{
			return 4;
			// увеличение силы
		}
	...
	private static boolean debuff(int dragonHP)
	{
		...
		for (int i = 1; i <= knightA / debuff; i++)
		{
			...
			newMin = ((float) knightHP / dragonA) + (((float) (knightA - i * debuff) / 
					(dragonHP - knightA)) * ((float) knightHP / dragonA));
